...
for row in movieReader:
    ...
    curs.execute('''INSERT OR REPLACE INTO movies(imdb_id, id, overview, genres, title, release_date, homepage, poster_path, tagline) VALUES(?,?,?,?,?,?,?,?,?)''', (row["imdb_id"], row["id"], row["overview"], genres, row["title"], row["release_date"], row["homepage"], row["poster_path"], row["tagline"]))
...
for row in starReader:
    ...
    for item in cast:
        if item["order"] < 7:
            curs.execute('''INSERT INTO casts(id, character, name, profile_path) VALUES(?,?,?,?)''', (row["id"], item["character"], item["name"], item["profile_path"]))
    for item in crew:
        if item["department"] == "Directing" or item["department"] == "Writing":
            curs.execute('''INSERT INTO crews(id, name, role) VALUES(?,?,?)''', (row["id"], item["name"], item["department"]))
